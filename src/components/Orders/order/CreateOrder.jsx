...
const CreateOrder = () => {
  ...
  const update = async () => {
    try {
      await axios.post(
        `${API_BASE_URL}/${state.order_id ? "updateOrder" : "createOrder"}`,
        {
          input: {
            ...computedState,
  ...
  const calculate = async () => {
    ...
    try {
      ...
      setState((prevState) => {
        return {
          ...prevState,
          order_id: data.data.insertId,
        };
      });
      await getOrdersDetails(data.data.insertId);
      MySwal.close();
      setIsLoading(false);
      getSummary();
      toast.success("Order updated successfully");
                  ...
                  <form action="">
                    ...
                    {isMinWeightError ? (
                      <p className="text-red-500">
                        <i className="mdi mdi-alert" /> Gross Weight doesn't
